...
sources= sources/main.cpp sources/comp.cpp\
sources/inputfuncs.cpp sources/outputfuncs.cpp sources/solvefuncs.cpp\
sources/testfunc.cpp sources/colors.cpp

flags=-ggdb3 -std=c++20 -O0 -Wall -Wextra -Weffc++ -Wc++14-compat -Wmissing-declarations -Wcast-align -Wcast-qual\
-Wconversion -Wctor-dtor-privacy -Wempty-body -Wfloat-equal -Wformat-nonliteral -Wformat-security -Wformat=2 -Winline\
...
all:
	$(CC) $(sources) -o quadr $(flags)
test:
	$(CC) $(sources) -o quadrTest -DTESTMODE $(flags)
debug:
	$(CC) $(sources) -o quadrDebug -DDEBUGMODE $(flags)
testdebug:
	$(CC) $(sources) -o quadrTestDebug -DDEBUGMODE -DTESTMODE $(flags)
